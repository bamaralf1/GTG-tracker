...
function renderExercicios() {
  try {
    ...
    dados.exercicios.forEach(e => {
      ...
      r && t && parseFloat(t) > 7 && (r.classList.add("show"), r.innerHTML = "⚠ Esforço médio alto hoje (acima de PESADO): " + t + ". Considere reduzir reps.")
  ...
  try {
    ...
    if (rpeWarnEl) {
      if (rpeVal && parseFloat(rpeVal) > 7) {
        ...
        rpeWarnEl.innerHTML = '⚠ Esforço médio alto hoje (acima de PESADO): ' + rpeVal + '. Considere reduzir reps.';
      } else {
    ...
    // Volume badge
    const volBadge = card.querySelector('.ex-volume-badge');
    const seriesHojeCount = regsHoje.length;
    if (volBadge) {
      volBadge.remove();
    }
    if (seriesHojeCount >= 12) {
      const badge = document.createElement('span');
      badge.className = 'ex-volume-badge excedido';
      badge.textContent = '⚠ ' + seriesHojeCount + ' séries';
      card.querySelector('.exercise-name')?.after(badge);
    } else if (seriesHojeCount >= 8) {
      const badge = document.createElement('span');
      badge.className = 'ex-volume-badge atingido';
      badge.textContent = '✅ ' + seriesHojeCount + ' séries';
      card.querySelector('.exercise-name')?.after(badge);
    }
...
function adicionarSerie(exId) {
  ...
  if (typeof iniciarRestTimer === "function" && !restTimer.rodando) {
    iniciarRestTimer(900, exId, ex.nome);
  }
}
